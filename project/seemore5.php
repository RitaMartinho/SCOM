...
<body>
    <?php
      ...
      foreach($lines as $hourminute){
        ...
        array_push($net_hour, array("loc"=>$firstelement[5],"netspeed"=>$firstelement[8]));
      }
      ...
      $myfile = fopen('txtfiles_todisplay/mediumspeed_by_loc.txt', 'w+') or die("Unable to open file!");
      foreach($mediumbyloc as $item){
        fwrite($myfile, $item."\n");
      }

      $myfile1 = fopen('txtfiles_todisplay/maxspeed_by_loc.txt', 'w+') or die("Unable to open file!");
      foreach($maxbyloc as $item1){
        fwrite($myfile1,$item1."\n");
      }

      $myfile2 = fopen('txtfiles_todisplay/minspeed_by_loc.txt', 'w+') or die("Unable to open file!");
    ...
    <script type="text/javascript"> //lost connections
        ...
        rawFile.open("GET", "txtfiles_todisplay/mediumspeed_by_loc.txt", false); // using synchronous call
        var allText;
        ...
        rawFile1.open("GET", "txtfiles_todisplay/maxspeed_by_loc.txt", false); // using synchronous call
        var allText1;
        //alert("Starting to read text");
        ...
        rawFile2.open("GET", "txtfiles_todisplay/minspeed_by_loc.txt", false); // using synchronous call
        var allText2;
        //alert("Starting to read text");
        ...
        let massPopChart = new Chart(myChart, {
          ...
          options:{
            title:{
              display:true,
              text:'FEUP Eduroam - Network Speed (in Kpbs) per location',
              fontSize:30
            },
            legend:{
              display:true,
